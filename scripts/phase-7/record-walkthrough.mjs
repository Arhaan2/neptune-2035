...
try {
  ...
  for (let index = 0; index < count; index++) {
    ...
    steps.push({ index, title, runId: await walkthrough.getAttribute('data-run-id'), displayedTimeS: await page.locator('main.twin-app').getAttribute('data-display-time'), inspectionStatus, screenshot: filename, text: await walkthrough.innerText() });
    if (index + 1 < count) await button('Next walkthrough step').click();
  }
