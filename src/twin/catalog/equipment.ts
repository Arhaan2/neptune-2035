...
export interface EquipmentConfiguration {
  ...
  specifications: ComponentSpecification[]; controlPolicy: typeof CONTROL_POLICY; workloadProfile: typeof TRAFFIC_PROFILE;
  economics: EconomicAssumptions;
}
const units: Record<string, string> = {capacityW:'W',shutoffPa:'Pa',freeFlowM3S:'m³/s',efficiency:'1',energyWh:'Wh',storageMaxW:'W',chargeEfficiency:'1',dischargeEfficiency:'1',accelerators:'count',heightU:'U',liquidCaptureFraction:'1',UAWPerK:'W/K',diameterM:'m',roughnessM:'m'};
...
export function resolveSpecification(design:Design,assetIdOrRole:string):ComponentSpecification {
  const equipment=equipmentFor(design),role=roles.includes(assetIdOrRole as EquipmentRole)?assetIdOrRole as EquipmentRole:roleForAsset(assetIdOrRole);
...
export function validateEquipment(design:Design):void {
  ...
  if(identity(e.workloadProfile)!==identity(TRAFFIC_PROFILE))failure('unsupported-configuration','WORKLOAD_PROFILE','Unknown workload profile version; network behavior is unchanged in Phase 2.');
