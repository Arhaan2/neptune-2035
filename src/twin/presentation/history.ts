...
export interface InspectionRequest { assetId: string; timeS: number; boundary: 'post' | 'previous' }
...
export async function resolveInspection(design: Design, source: SimulationState, request: InspectionRequest, options: { yieldTask?: () => Promise<void>; cancelled?: () => boolean; now?: () => number } = {}): Promise<InspectionResolution> {
  ...
  if (!Number.isFinite(request.timeS) || request.timeS < 0 || request.timeS > source.timeS || !['post', 'previous'].includes(request.boundary)) return { ...base, reason: 'History time must be within the retained source observation. No nearest sample is substituted.' };
  ...
  const observe = (timeS: number, copy: () => SimulationState) => {
    const eligible = request.boundary === 'previous' ? timeS < request.timeS : timeS <= request.timeS;
    if (!eligible || timeS === lastSampleTime) return;
    const copied = copy();
    if (request.boundary === 'previous' || timeS === request.timeS) snapshot = copied;
    if (moduleId) {
