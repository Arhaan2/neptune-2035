...
export function validateDecisionResult(campaign:DecisionCampaign,value:unknown):asserts value is DecisionResult {
  ...
  for(const run of result.runs){
    ...
    if(run.state!==null&&identity(run.recovery)!==identity(experimentRecoveryReport(run.state)))throw Error('Recovery report differs from raw whole-run recovery evidence.');
...
export function decisionReport(campaign:DecisionCampaign,result:DecisionResult):string {
