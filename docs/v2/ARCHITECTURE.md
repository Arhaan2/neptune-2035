...
Supported projects carry validated reference-family parameters, not arbitrary editable CAD or graph structures. Electrical operation remains radial. Legacy Generation III designs retain open ties and no transfer benefit. The explicit Phase 5 transfer preset adds installed isolators, receiving buses and normally-open ties; a deterministic one-hop, open-before-close policy may supply eligible whole platforms from a declared donor when shared capacity permits. Active connections change without reparenting physical equipment. No energized mesh, parallel sources or automatic retransfer is supported. Hydraulics supports declared series circuits with identical parallel pump branches. Network traffic is a declared demand screen, not packet simulation. The [Phase 5 contract](../phase-5/CONTRACT.md) and [requirement matrix](../phase-5/REQUIREMENT-MATRIX.md) define its bounded acceptance envelope.
