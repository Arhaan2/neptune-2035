...
The enabled power graph must contain one radial path from each energized module domain to `shore/grid`. The engine traverses upstream transformer, switchboard and connection capacities. A missing native path, cycle or enabled multiple-parent tie is unsupported and yields zero grid capacity plus a visible warning. Generation I shares the centralized shore transformer/bus; II and III have platform domains. All share the same upstream source. Legacy Generation III normally-open ties remain non-transferring. Each module's reference battery/distribution branch is capped at 2.2 MW, subject to its installed specification. Legacy grid allocations remain proportional within domain and common-source limits; optional charging receives only remaining branch/domain/source capacity.

The explicit Phase 5 transfer preset selects model `neptune-transfer-1` and timing interpretation `transfer-boundary-1`. It isolates an eligible failed original feeder path before closing a single declared donor tie. Whole-platform requests use declared priority and stable IDs under native-first shared resource accounting; transfer creates no new supply. Partial restoration admits separate whole platforms and refuses indivisible bundles that do not fit. Failed buses/downstream equipment and unavailable required network or cooling still constrain actual service. Cleared original faults do not automatically reconnect after transfer, and failed or capacity-shed transfers lock out until a fresh run. Internal transfer deadlines may split the ordinary integration step; persisted external event/checkpoint timing retains the existing compatibility boundary. This is a simulated deterministic capacity policy, not AC load flow, protection coordination, a general restoration optimizer or real equipment control. New generic switch ratings, mass, dimensions and included prices are assumptions; actuator/standby power, incremental switch losses and physical switching transients are excluded. See the [Phase 5 contract](../phase-5/CONTRACT.md) for exact policy, evidence and limits.
...
`conservationResiduals(design,state)` retains signed aggregate W and normalized residuals and also reports the sum and maximum of absolute module residuals for electrical and thermal accounting. The aggregate electrical denominator is `max(1 W, networkAuxiliaryGridW + sum(module.gridW + module.batteryDischargeW / eta_discharge))`, representing total input power including stored-energy depletion. The aggregate thermal denominator is `max(1 W, networkAuxiliaryGridW + sum(module.facilityW))`, all declared IT and auxiliary heat sources. Signed totals and sums of absolute residuals use these aggregate denominators. Root/platform network auxiliary input is separately assessed; the component residual measures cover module balances.

`electricalMaxAbsoluteNormalized` is the maximum of each absolute module electrical residual divided by `max(1 W, module.gridW + module.batteryDischargeW / eta_discharge)`; `thermalMaxAbsoluteNormalized` analogously uses `max(1 W, module.facilityW)`. Normalizing each component before taking the maximum prevents another module’s input power from diluting its error. Absolute sums and maxima prevent opposing errors from cancelling. These reporting normalizations are explicit and differ from conservative per-module runtime denominators. Zero-input cases use the 1 W floor. Phase 8 requires each module’s absolute residual <=1e-5 W and normalized residual <=1e-9; no aggregate can replace those component checks. The added reporting fields change no solver arithmetic, runtime tolerance, checkpoint schema, or solver version. None of these residuals measures physical model uncertainty.

A final long-outage audit additionally verifies that provisional pump-controller commands cannot be logged as powered transitions before electrical correction. A 1,800-second depletion/recovery regression preserves the original feeder command, logs one actual power loss/recovery per running pump, rejects repeated false enable messages, and matches split versus direct replay exactly. The final combined test count is in RELEASE-CANDIDATE.md.
