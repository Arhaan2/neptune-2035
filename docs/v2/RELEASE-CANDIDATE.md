...
The user requested integration into the existing repository. The source will be fast-forward merged into `codex/neptune`; the separate candidate will occupy `v2-preview/` on the existing Pages deployment branch. The v0.1 root payload and baseline tag must remain byte-for-byte unchanged. Hosted acceptance and the returned real preview URL are recorded after deployment below.
...
## Preview deployment and rollback plan

Publish the validated compiled payload under `v2-preview/` in a fresh normal commit descended from the existing `codex/pages` deployment. Compare every pre-existing root file against the preserved deployment before pushing. Fast-forward the candidate source into the repository’s default `codex/neptune` branch as requested. Verify the actual served preview release/file manifest and run the browser suite against that real subpath. Record the source, artifact and new Pages deployment identities here. No force-push, root-payload replacement or new repository is part of this integration.
...
Production remains `https://arhaan2.github.io/neptune-2035/`, served from deployment `672805a984feadaab375061817c919fd6849b311` with application source `127a2fade93112d58f8edd9f6fc6e9e02b12b57c`; annotated `v0.1.0` points to source checkpoint `8b78638838a82ee603fdb001bfc978df49a9665d`. To abandon this candidate, leave the production deployment untouched. If a future approved release needs rollback, restore the preserved compiled deployment in a new normal commit, rebuild Pages, verify `release.json` and rerun hosted acceptance. Never rewrite shared history.

Production promotion requires explicit user approval. Publishing a separate preview would not constitute promotion or empirical validation.

## Local handoff

The compiled candidate is served for review at `http://127.0.0.1:5175/` while this local preview process runs. This loopback address is not a hosted/public candidate URL. The final package, tested code and deployment instructions are concrete and reviewable; the user’s request to merge into the existing repository now authorizes the remaining integration and separate preview publication there.
