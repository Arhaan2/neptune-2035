...
Implementation acceptance of the previously recorded local source: **PASS**. The subsequent Pause repair and browser execution corrections have passed independent review and 33 focused cases; full acceptance of the new candidate remains pending. Integration status: **NOT MERGED**. The PR/final report and post-merge machine-readable record must supply the actual tested PR head, pre-merge main, merge/main SHAs, CI links and clean post-merge result; this document does not pre-claim integration.
...
The first current Apple Silicon runs passed all 256 unit cases but failed browsers: push 64/75 (Firefox 15/25, WebKit 24/25), PR 67/75 (Firefox 17/25, WebKit 25/25); Chromium passed both. Measurements were not executed. The push-only WebKit failure contained explicit Metal host-memory allocation errors. A separately reproduced application defect disabled Pause during pending clock work; its repair freezes the last UI-committed checkpoint and rejects delayed replies. The new real-worker regression passed in all three browsers, alongside all Phase 1 cases and the original clock test (33/33). Firefox now runs headed, WebKit uses the other desktop projects' DPR1 at the unchanged CSS viewport, generated reports are excluded from dev-server watching, and text failure contexts are retained. Assertions, deadlines, telemetry and all original cases remain. These settings do not prove a single cause for the earlier Firefox failures. The next clean full gate contains 256 unit and 78 browser cases.
