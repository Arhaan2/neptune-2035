...
Pause remains available while a clock advance is pending. Pause and manual Cancel synchronously stop interval dispatch, invalidate the pending generation and freeze the last UI-committed checkpoint, including against already-delayed progress, completion and cancellation replies. Controls unlock against that chosen checkpoint; a later worker acknowledgement cannot change the base for the next manual step. Cancel persists that checkpoint and preserves the interrupted target for explicit Resume, which sends the checkpoint back to the worker. Direct worker cancellation still returns the worker's latest validated partial state to its caller. Start is unavailable during pending manual work.
