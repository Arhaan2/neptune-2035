...
Pause remains available while a clock advance is pending. It synchronously stops interval dispatch, invalidates the pending generation and freezes the last UI-committed checkpoint, including against already-delayed completion and cancellation replies. Resume sends that checkpoint back to the worker. Start is unavailable during pending manual work; manual replay cancellation separately retains its acknowledged completed progress.
