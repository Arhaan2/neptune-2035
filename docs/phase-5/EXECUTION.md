...
PH5-L01 repair `6c19ac2` (integrated `08f6b23`) keeps the experiment form mounted but natively hidden while Compare is active. Only two UI files change; physical engine behavior and all retained acceptance tests are unchanged. Testing's additional `272be2c` regression waits for the actual view transition, checks all selected draft values, and inspects the new worker export. Fixing's actual before/after exports reproduce the old 80 accelerator-second mistaken disturbance and the repaired zero-shortfall healthy 40 s run. A fresh frozen candidate receives complete gates; no failed gate is relabeled as a pass.

Testing independently reproduced PH5-L01 on the original product with the explicit regression, then passed the unchanged regression on `6c19ac2` with a real healthy 40 s export, 7 s dwell, no disturbances, zero shortfall and no browser errors or failed requests. Verification independently reviewed all four before/after exports and the exact source repair. Both closed PH5-L01; the first failed gate remains preserved. All specialist browser servers were stopped before the final complete local gate.
