...
Recovery requires all declared service and thermal predicates continuously satisfied for a serialized dwell interval. Any relapse resets the candidate. Persist the qualifying interruption/reference event, candidate onset and confirmation boundary; distinguish onset elapsed time from confirmation elapsed time. Persisted `referenceTimeS` is qualifying violation onset; reports label that origin explicitly and separately resolve the recorded fault event time and event-relative recovery latency. In the signature, fault30 s precedes violation240 s, recovery364 s and confirmation369 s. Record repeated interruptions; an earlier recovery never hides a later unresolved episode. Distinguish no qualifying interruption, recovered, not recovered within completed observation and incomplete observation. Confirmation at terminal T requires its post-event boundary still qualifying; an unfinished dwell is not recovery.
